...
# 数据集划分并生成 DataLoader
def create_dataloaders(droplet_data, labels, batch_size, test_size=0.15, val_size=0.15):
    # 首先划分训练集和测试集
    X_train, X_test, y_train, y_test = train_test_split(droplet_data, labels, test_size=test_size, random_state=42)
    ...
    # 转换为 PyTorch 的张量
    train_data = TensorDataset(torch.tensor(X_train_std, dtype=torch.float32), torch.tensor(y_train, dtype=torch.long))
    val_data = TensorDataset(torch.tensor(X_val_std, dtype=torch.float32), torch.tensor(y_val, dtype=torch.long))
    test_data = TensorDataset(torch.tensor(X_test_std, dtype=torch.float32), torch.tensor(y_test, dtype=torch.long))
