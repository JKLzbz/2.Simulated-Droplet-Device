...
class ResidualNetwork(nn.Module):
    def __init__(self, input_channels, out_channels, num_residual_layers):
        ...
        # 残差块
        self.residual_blocks = self.make_residual_layers(out_channels, num_residual_layers)

        # 平均池化层
        self.avg_pool = nn.AdaptiveAvgPool1d(1)  # 自适应池化到1

    def make_residual_layers(self, input_channels, num_residual_layers):
        layers = []
        in_channels = input_channels
        for i in range(num_residual_layers):
            out_channels = in_channels if i == 0 else in_channels * 2
            layers.append(ResidualBlock(in_channels, out_channels, stride=2 if i > 0 else 1))
