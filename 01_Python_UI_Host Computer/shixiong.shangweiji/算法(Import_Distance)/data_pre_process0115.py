...
def read_csv_data(filename):
    ...
    labels = df.iloc[:, 2113].values  # 第2114列是标签
    data = df.iloc[:, 0:2113].values  # 第1列到第2113列为所有传感器数据
...
# 处理范围映射
def process_range_data(distance_data):
    # 定义范围编码
    distance_ranges = [
        (0, 40), (40, 65), (65, 90), (90, 120), (120, 150), (150, float('inf'))]
    # 映射到索引
...
def split_data(time_series_data, time_series_labels):
    ...
    train_data, temp_data, train_labels, temp_labels = train_test_split(time_series_data, time_series_labels,
                                                                        test_size=0.3, random_state=42)
    val_data, test_data, val_labels, test_labels = train_test_split(temp_data, temp_labels, test_size=0.5,
                                                                    random_state=42)
    # 分割数据列
    # 训练集
    train_droplet_data = train_data[:, 0:704]
    train_acc_data = train_data[:, 704:1408]
    train_gyro_data = train_data[:, 1408:2112]  # 第1409到2112列是gyro_data
    train_distance_data = train_data[:, 2112]  # 第2113列是distance_data

    # 验证集
    val_droplet_data = val_data[:, 0:704]
    val_acc_data = val_data[:, 704:1408]
    val_gyro_data = val_data[:, 1408:2112]  # 第1409到2112列是gyro_data
    val_distance_data = val_data[:, 2112]  # 第2113列是distance_data

    # 测试集
    test_droplet_data = test_data[:, 0:704]
    test_acc_data = test_data[:, 704:1408]
    test_gyro_data = test_data[:, 1408:2112]  # 第1409到2112列是gyro_data
    test_distance_data = test_data[:, 2112]  # 第2113列是distance_data

    return train_droplet_data, train_acc_data, train_gyro_data, train_distance_data, \
        val_droplet_data, val_acc_data, val_gyro_data, val_distance_data, \
        test_droplet_data, test_acc_data, test_gyro_data, test_distance_data, \
...
def process_data(filename, num_batch_size):
    ...
    # 步骤2：# 数据集划分
    train_droplet_data, train_acc_data, train_gyro_data, train_distance_data, \
        val_droplet_data, val_acc_data, val_gyro_data, val_distance_data, \
        test_droplet_data, test_acc_data, test_gyro_data, test_distance_data, \
    ...
    # 步骤4：距离及温度范围映射
    train_distance_indices = process_range_data(train_distance_data)
    val_distance_indices = process_range_data(val_distance_data)
    test_distance_indices = process_range_data(test_distance_data)
    ...
    # 创建数据加载器
    train_dataset = TensorDataset(train_time_series_data, train_labels, train_distance_indices)
    val_dataset = TensorDataset(val_time_series_data, val_labels, val_distance_indices)
    test_dataset = TensorDataset(test_time_series_data, test_labels, test_distance_indices)
...
class CNN_GRU_Transformer(nn.Module):
    def __init__(self):
        super().__init__()


def main():
    process_data('data_sheet.csv', 64)
