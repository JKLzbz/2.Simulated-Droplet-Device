...
class FeatureFusion(nn.Module): #增加了多头注意力机制
    def __init__(self, resnet_dim, gru_dim, fusion_dim=256, num_heads=4):
        super().__init__()
        # 初始化参数
        ...
        self.attention = nn.MultiheadAttention(
            embed_dim=fusion_dim,  # 输入特征维度
            num_heads=num_heads,  # 注意力头数
            dropout=0.3,  # 与原始结构保持一致
            batch_first=True  # 输入格式为 (batch, seq, feature)
        )
        self.norm1 = nn.LayerNorm(fusion_dim)  # 层归一化

        # ------------------- 改进后的门控生成器 -------------------
        self.fusion_gate = nn.Sequential(
            nn.Linear(fusion_dim * 3, fusion_dim),  # 输入维度扩大
            Mish(),
            nn.Linear(fusion_dim, 2),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, res_feat, gru_feat):
        ...
        attn_output = self.norm1(combined + attn_output)  # 残差连接

        # 步骤4：分离注意力增强后的特征
        attn_res = attn_output[:, 0, :]  # (B,256)
        attn_gru = attn_output[:, 1, :]  # (B,256)

        # 步骤5：改进的门控融合
        interaction = attn_res * attn_gru  # 新增交叉项
        gate_input = torch.cat([attn_res, attn_gru, interaction], dim=1)  # (B,768)
        gate = self.fusion_gate(gate_input)  # (B,2)

        # 步骤6：加权融合（保留原始特征）
        fused = gate[:, 0:1] * (res_feat + attn_res) + gate[:, 1:2] * (gru_feat + attn_gru)
        return fused
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'Res_1DCNN_BiGRU_Attention_Multi_Signal_best_model.pth')
...
def test_model(model, test_loader):
    model.load_state_dict(torch.load('Res_1DCNN_BiGRU_Attention_Multi_Signal_best_model.pth'))
    model.eval()
    all_predicts = []
    all_labels = []
    correct = 0
...
def main():
    ...
    # 打印模型结构信息
    print_model_architecture(model, (1, 1, 704))  # 输入形状为 (channels=3, seq_len=704)
    ...
    # 训练模型
    train_loss, val_loss, train_acc, val_acc, lr_history = train_model(model, train_loader, val_loader, criterion, optimizer, 10)
    # 保存训练结果到Excel（新增部分）
    df = pd.DataFrame({
    ...
    df.to_excel('Res_1DCNN_BiGRU_Attention_Multi_Signal_training_results.xlsx', index=False)
