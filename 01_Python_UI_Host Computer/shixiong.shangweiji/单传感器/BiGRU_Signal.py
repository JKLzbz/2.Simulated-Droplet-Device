...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'BiGRU_Signal_best_model.pth')
...
def test_model(model, test_loader):
    model.load_state_dict(torch.load('BiGRU_Signal_best_model.pth'))
    model.eval()
    all_predicts = []
    all_labels = []
    correct = 0
...
def main():
    ...
    df.to_excel('BiGRU_Signal_training_results.xlsx', index=False)
