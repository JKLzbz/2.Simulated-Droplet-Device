...
class TimeSeriesTransformer(nn.Module):
    def __init__(self, input_size, d_model=48, nhead=2, num_layers=1,  # 减少模型复杂度
                 dim_feedforward=128, dropout=0.6, num_classes=4):  # 增大Dropout
        super().__init__()
        self.input_proj = nn.Sequential(
            nn.Linear(input_size, d_model),
            nn.LayerNorm(d_model),  # 添加层归一化
            nn.Dropout(dropout)
        )
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            epochs_no_improve = 0
            torch.save(model.state_dict(), 'transformer_ceshi_best_model.pth')
        else:
            epochs_no_improve += 1
...
def test_model(model, test_loader):
    model.load_state_dict(torch.load('transformer_ceshi_best_model.pth'))
    model.eval()
    all_predicts = []
    all_labels = []
    correct = 0
...
def main():
    ...
    df.to_excel('Transformer_training_results.xlsx', index=False)
