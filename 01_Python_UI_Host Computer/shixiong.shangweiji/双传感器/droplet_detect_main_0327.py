...
from 双传感器.Res_1DCNN_BiGRU_Attention_Muti import CombinedModel
from Mode_1DCNN import CNN1D
from 双传感器.double_data_process111 import process_data
...
class DataSaveThread(QThread):
    ...
    def __init__(self, record_result: QLineEdit):
        ...
        # 加载标准化器和模型
        self.scaler_droplet = joblib.load('scaler_droplet.pkl')
        self.scaler_acc = joblib.load('scaler_acc.pkl')
        self.scaler_gyro = joblib.load('scaler_gyro.pkl')
        self.model = self.load_model()
    ...
    def load_model(self):
        ...
        # model = CNN1D(in_channel=3, num_classes=4, dropout_rate=0.5)
        # 加载训练好的权重
        model.load_state_dict(torch.load(
            'Res_1DCNN_BiGRU_Attention_Multi_zengqiang_best_model.pth',
    ...
    def save_to_csv(self):
        ...
        # 1. 准备输入数据
        features = np.stack([
            self.droplet_save_buffer,
            self.sum_acc_save_buffer,
            self.sum_gyro_save_buffer
        ], axis=0)  # (3, 704)

        # 2. 数据标准化
        features[0] = self.scaler_droplet.transform(features[0].reshape(1, -1)).flatten()
        features[1] = self.scaler_acc.transform(features[1].reshape(1, -1)).flatten()
        features[2] = self.scaler_gyro.transform(features[2].reshape(1, -1)).flatten()

        # 3. 转换为模型输入格式 (batch_size=1, channels=3, seq_len=704)
        input_tensor = torch.tensor(features, dtype=torch.float32).unsqueeze(0)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        input_tensor = input_tensor.to(device)  # 将输入张量移动到模型所在设备

        # 4. 进行预测
        with torch.no_grad():
            outputs = self.model(input_tensor)
            _, predicted = torch.max(outputs, 1)
            class_result = predicted.item()

        # 5. 转换类别为可读标签
        class_names = ["稳定状态下飞沫信号", "自呼吸干扰信号", "气流扰动信号", "运动状态下飞沫信号"]
        result = f"{class_names[class_result]}"
        # 添加结果判断和信号发射
        if class_result in [0, 3]:           #开摄像头
            self.send_result_signal.emit(1)  # 发送数字1    开摄像头
...
class KalmanFilter:
    def __init__(self, process_var=0.01, measure_var=0.1):
        """
        单变量卡尔曼滤波器
        :param process_var: 过程噪声方差 (Q)
        :param measure_var: 测量噪声方差 (R)
        """
        self.Q = process_var
        self.R = measure_var
        self.x = 0      # 初始状态估计
...
class DataProcessThread(QThread):
    ...
    def __init__(self):
        ...
        # 添加卡尔曼滤波器实例
        self.kalman_acc = KalmanFilter(process_var=0.001, measure_var=0.1)
        self.kalman_gyro = KalmanFilter(process_var=0.001, measure_var=0.1)
        self.filter_mutex = QMutex()  # 滤波器互斥锁
    ...
    def notify_update_data(self, data):
        with QMutexLocker(self.data_mutex):  # 加锁，保护数据
            ...
            # 使用卡尔曼滤波
            with QMutexLocker(self.filter_mutex):
                filtered_acc = self.kalman_acc.update(raw_acc)
                filtered_gyro = self.kalman_gyro.update(raw_gyro)

            self.droplet_data = data['droplet']
            self.acc_data = filtered_acc  # 使用滤波后的加速度
            self.gyro_data = filtered_gyro  # 使用滤波后的陀螺仪
