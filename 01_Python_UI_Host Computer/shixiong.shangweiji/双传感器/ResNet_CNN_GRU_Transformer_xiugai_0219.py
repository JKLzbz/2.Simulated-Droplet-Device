...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, early_stopping_patience):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'Res_1DCNN_BiGRU_jiandan_best_model.pth')
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1
...
def test_model(model, test_loader):
    model.load_state_dict(torch.load('Res_1DCNN_BiGRU_jiandan_best_model.pth'))
    model.eval()
    all_predicts = []
    all_labels = []
    correct = 0
...
def main():
    ...
    df.to_excel('Res_1DCNN_BiGRU_jiandan_training_results.xlsx', index=False)
