...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   #define TEST_CONNECTION_TYPE         SOCKET_TYPE_TCP_SERVER                                     //           SOCKET_TYPE_UDP=0=WIFI module as UDP, SOCKET_TYPE_TCP_CLIENT=1=WIFI module as TCP Client, SOCKET_TYPE_TCP_SERVER=2=WIFI module as TCP Server
                                                                                                   // (Chinese: SOCKET_TYPE_UDP=0=WIFI模组做UDP, SOCKET_TYPE_TCP_CLIENT=1=WIFI模组做TCP客户端, SOCKET_TYPE_TCP_SERVER=2=WIFI模组做TCP服务器

   /////////////
   ////	 Macros for Soket ip:port pairs  (Chinese：套接字的本地端口和目标地址目标端口的宏定义) 
   //local port	(Chinese：套接字的本地端口)
#if (TEST_CONNECTION_TYPE==SOCKET_TYPE_TCP_CLIENT)                                                 // if module as TCP Client (Chinese:如果模组作为TCP客户端)
...
#elif (TEST_CONNECTION_TYPE==SOCKET_TYPE_UDP) || (TEST_CONNECTION_TYPE==SOCKET_TYPE_TCP_SERVER)    // if module as UDP or TCP Server (Chinese:如果模组作为UDP或TCP服务器)
...
#error WRONG TEST_CONNECTION_TYPE defined !
   ...
   /////////////
   //Remote address and port  (Chinese：套接字的目标地址和目标端口)
#if (TEST_CONNECTION_TYPE==SOCKET_TYPE_UDP)                                                        // if module as UDP (Chinese:如果模组作为UDP，则可以指定目标地址和端口，也可以随便填充，在发送数据时再设置或更改)
   #define TEST_REMOTE_ADDR           "192.168.4.2"
   #define TEST_REMOTE_PORT            1234
#elif (TEST_CONNECTION_TYPE==SOCKET_TYPE_TCP_CLIENT)                                               // if module as TCP Client (Chinese:如果模组作为TCP客户端，当然必须指定目标地址和目标端口，即模组所要去连接的TCP服务器的地址和端口)
   #define TEST_REMOTE_ADDR           "192.168.43.233"  // "www.baidu.com"
   #define TEST_REMOTE_PORT            1234  // 80
#elif (TEST_CONNECTION_TYPE==SOCKET_TYPE_TCP_SERVER)                                               // if module as TCP Server (Chinese:如果模组作为TCP服务器)
...
#if ( 1 && ((TEST_CONNECTION_TYPE==SOCKET_TYPE_TCP_CLIENT)||(TEST_CONNECTION_TYPE==SOCKET_TYPE_TCP_SERVER)) )   //If you hope to change TCP Windows, please change '0' to '1' in the #if clause before setup the connection
...
#define TEST_SEND_DATA_SIZE   2920 //5840 //2048 // 1024   // If using large size of array, ensure system stack is large engough for the array variable。 Or stack over-bound leakage might bring about the mcu into "hardware fault exception"
   ...
   for(batch=0; ; batch++)
   {
        ...
        {                                                 // (Chinese: 持续发送一段时间后，观察表达式 MBytes*1024*1024+total_sent 的值，和接收端接收到的数据个数进行比较，可以粗略衡量模组的丢包率。)
              ...
              LED_set(0, MBytes&0x01); LED_set(1, 1-(MBytes&0x01));	
#endif				
        }
...
#else // using M8266WIFI_SPI_Send_Data() to sent data packet by packet (Chinese: 调用M8266WIFI_SPI_Send_Data() 来一个一个包的发送数据，实际几乎就是M8266WIFI_SPI_Send_BlockData()的函数实现代码
 {
        ...
        {		
            sent += M8266WIFI_SPI_Send_Data(snd_data+sent, , link_no, &status);
            if(sent>=len)  break;
            if( (status&0xFF) == 0x00 )
            {
                loops = 0;
