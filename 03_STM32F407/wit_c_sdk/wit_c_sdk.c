...
static uint8_t __CaliSum(uint8_t *data, uint32_t len)//校验和计算
{
    uint32_t i;
    uint8_t ucCheck = 0;
...
void CopeWitData(uint8_t ucIndex, uint16_t *p_data, uint32_t uiLen)
{
	...
	if(uiReg1Len)
	{
		memcpy(&sReg[uiReg1], p_usReg1Val, uiReg1Len<<1);//8位为1个字节，*p_usReg1Val为16位数据，所以搬移的个数需要乘2，乘2用左移符号表示
		p_WitRegUpdateCbFunc(uiReg1, uiReg1Len);//由于p_WitRegUpdateCbFunc为函数指针，所以该代码等效于执行SensorDataUpdata(uiReg1, uiReg1Len)
	}
	if(uiReg2Len)
	{ 
...
void WitSerialDataIn(uint8_t ucData)
{
    ...
    switch(s_uiProtoclo)
    {
        case WIT_PROTOCOL_NORMAL:
            if(s_ucWitDataBuff[0] != 0x55)//找寻一帧数据的帧头（见WIT私有协议-协议格式）
            {
                s_uiWitDataCnt--;
            ...
            {
                ucSum = __CaliSum(s_ucWitDataBuff, 10);//SUMCRC=0x55+TYPE+DATA1L+DATA1H+DATA2L+DATA2H+DATA3L+DATA3H+DATA4L+DATA4H
                if(ucSum != s_ucWitDataBuff[10])//计算出的校验和与传感器输出的校验和不相等，说明当前获取的一帧数据是错误的
                {																//继续剔除第0个元素，将后面的元素左移一个数组单位
                    s_uiWitDataCnt--;
                    memcpy(s_ucWitDataBuff, &s_ucWitDataBuff[1], s_uiWitDataCnt);
                    return ;
                }
								////帧头对的，且计算出的校验和与传感器输出的校验也相等，说明找到一组有效的数据
...
//                CopeWitData(s_ucWitDataBuff[1], usData, 4);//官方注释的，它将该代码放到了while循环函数中
                s_uiWitDataCnt = 0;
								ucRegIndex = s_ucWitDataBuff[1];//数据内容标签（0x55：时间，0x51:加速度，0x52:角速度，0x53:角度，0x54:磁场，0x55:端口状态，0x56:气压高度，0x57:经纬度，0x58:地速，0x59:四元数，0x5A:GPS定位精度，0x5F:读取）
								memcpy(usRegDataBuff,usData,8);//将四个维度的数据搬移到usRegDataBuff中
								uiRegDataLen = 4;//获取的数据，其长度设置为4
            }
        break;
